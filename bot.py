...
async def new_project(ctx):
	...
	#input() komudu gibi! (alttaki check fonksiyonu).
    def check(msg):
        return msg.author == ctx.author and msg.channel == ctx.channel

    name = await bot.wait_for('message', check=check)		#Projenin ismi alınır, bir input fonksiyonu gibi bir karşılık girişi verilmesi beklenir.
    data = [ctx.author.id, name.content]		#Kullanıcı İD'si ve ismi alınır. DB'de ikinci sütun (değişik sayı).
    await ctx.send("Lütfen projeye ait bağlantıyı gönderin!")
    link = await bot.wait_for('message', check=check)		#name ile aynıdır fakat isim yerine projenin linkini alır.
    data.append(link.content)		#data'ya alınan linki ekler.
...
async def get_projects(ctx):
    user_id = ctx.author.id		#Bütün projelerin isimlerini döndürür.
    projects = manager.get_projects(user_id) #Bütün projelerin linklerini döndürür.
    if projects:
        text = "\n".join([f"Project name: {x[2]} \nLink: {x[4]}\n" for x in projects])		#DB'deki proje isimlerini tek tek gezerek text içinde depolar.
        await ctx.send(text)	
    else:
...
async def skills(ctx):
    ...
    if projects:
        projects = [x[2] for x in projects]		#DB'deki üçüncü satırdam isimlere erişerek beceri eklemek istediğimiz projeyi soruyor.
...
async def update_projects(ctx):
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects:		#projects null değilse attributes'daki anahtarları birleştirerek döndürür.
        projects = [x[2] for x in projects]
