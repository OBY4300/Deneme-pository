...
values(?, ?, ?, ?, ?)""" # Doğru SQL sorgusunu buraya girin
		self.__executemany(sql, data)
...
WHERE user_id = ?""" # Doğru SQL sorgusunu buraya girin
...
WHERE project_name = ? AND user_id = ?""" # Doğru SQL sorgusunu buraya girin
		self.__executemany(sql, [data]) 


	def delete_project(self, user_id, project_id):
		sql = """DELETE FROM projects 
WHERE user_id = ? AND project_id = ? """ # Doğru SQL sorgusunu buraya girin
		self.__executemany(sql, [(user_id, project_id)])
	
	def delete_skill(self, project_id, skill_id):
		sql = """DELETE FROM skills 
WHERE skill_id = ? AND project_id = ? """ # Doğru SQL sorgusunu buraya girin
...
if __name__ == '__main__':
	...
	manager.insert_project([(1, "Discord Bot", "www.com.tr", "aciklama", manager.get_status_id("Prototip Oluşturma")),(1, "Saat_Belirleyici", "wwww.com.tr", "aciklama2", manager.get_status_id("Güncellendi"))])
	manager.insert_skill(1, "Discord Bot", "Python")
	manager.insert_skill(1, "Saat_Belirleyici", "APİ")
	manager.insert_skill(1, "Zmanzingonu", "Python-2")
	manager.insert_skill(1, "Discord Bot-2", "APİ-2")

	info = manager.get_project_info(1, "Discord Bot")
	print("Proje Bilgisi:", info)
	skills = manager.get_project_skills(1, "Saat_Belirleyici")
	print("Başka bir projede sergilenen yetenekler:", skills)
